...
# ─────────────────────────────────────────────────────────────────────────────
Gc_cases = [
    ("Gc(s) = s",           control.TransferFunction([1, 0], [1])),         # s
    ("Gc(s) = s^2",         control.TransferFunction([1, 0, 0], [1])),      # s^2
    ("Gc(s) = s^2/(s+20)",  control.TransferFunction([1, 0, 0], [1, 20])), # s^2 / (s+20)
]
...
for ax, (label, OL, T_total) in zip(axes, results):
    ...
    # Set axes limits to focus on the origin (Fixing the "weird graph" issue)
    ax.set_xlim([-50, 10])
    ax.set_ylim([-30, 30])
